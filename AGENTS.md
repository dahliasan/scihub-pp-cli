# Sci-Hub Printed CLI Agent Guide

This directory is a generated `scihub-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
scihub-pp-cli doctor --json
scihub-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
scihub-pp-cli which "<capability>" --json
scihub-pp-cli <command> --help
```
...
```bash
scihub-pp-cli <command> --help
scihub-pp-cli <command> --dry-run --agent
```

Use `--yes --no-input` only after the target, arguments, and side effects are clear.

For install and auth, read `README.md`. For longer agent-facing usage guidance (command reference, error codes, agent mode conventions), see `researchskills`' [find-pdf/references/scihub.md](https://github.com/dahliasan/researchskills/blob/main/skills/find-pdf/references/scihub.md). This file intentionally stays small so repo-local agents get invariant local guidance without duplicating those docs.
