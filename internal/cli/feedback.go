...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".scihub-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("SCIHUB_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("SCIHUB_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "scihub-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.scihub-pp-cli/feedback.jsonl.
When ` + "`SCIHUB_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`SCIHUB_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  scihub-pp-cli feedback list
  scihub-pp-cli feedback list --limit 5
  scihub-pp-cli feedback list --json`,
