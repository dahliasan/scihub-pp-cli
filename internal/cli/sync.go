...
Resource scoping:
  ...
  scihub-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  scihub-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  scihub-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  scihub-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  scihub-pp-cli sync --latest-only`,
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/scihub-pp-cli/data.db)")
